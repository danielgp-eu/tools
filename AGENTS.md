...
- **tools-core**: Shared utility library (19+ classes) for file ops, databases, JSON/XML, logging, timing, environment capture
...
```bash
# From workspace root (C:\www\Data\GitRepositories\GitHub\danielgp-eu\Java\tools)
...
| `tools-core/src/main/java/org/danielgp_eu/tools/core/*` | Core utility classes (BasicStructure, FileOps, JsonOps, etc.) |
| `tools-core/src/test/java/org/danielgp_eu/tools/core/FileOperationsClassTest.java` | Example JUnit 5 tests |
| `tools-archiving/src/main/java/org/danielgp_eu/tools/archiving/Application.java` | Entry point; picocli @Command |
| `tools-json_split/src/main/java/org/danielgp_eu/tools/json_split/Application.java` | Entry point; picocli @Command |
...
- Log files stored in `tools-core/logs/` (org_danielgp_eu__tools-*.log pattern)
...
| OSHI Core FFM | 7.3.0 | OS info capture (system memory, CPU, etc.) | compile |
...
1. Create class in `tools-core/src/main/java/org/danielgp_eu/tools/core/NewUtilityClass.java`
2. Organize methods into inner static SubClasses (e.g., `RetrievingSubClass`, `ProcessingSubClass`)
3. Add unit tests in `tools-core/src/test/java/org/danielgp_eu/tools/core/NewUtilityClassTest.java`
